...
struct ContentView : View {
    ...
    @State private var displayPlay: Bool = false
    ...
    var body: some View {
        VStack {
            ...
            .onTapGesture {
                print("点击了")
                self.displayPlay = true
                DispatchQueue.main.asyncAfter(deadline: .now() + 5) {
                    self.displayPlay = false
                }
            }
            .overlay(
                Button(action: {
                    ...
                    if self.displayPlay {
                        if self.play {
                            Image(systemName: "pause.fill").resizable().frame(width: 20.0, height: 20.0)
                        } else {
                            Image(systemName: "play.fill").resizable().frame(width: 20.0, height: 20.0)
                        }
                    }
                }).frame(width: 50, height: 50, alignment: .center).background(Color("Shadow"))
                    .padding([.bottom,.trailing],20),alignment: .bottomTrailing
            )
            ...
            HStack {
                ...
                Text("\(time.seconds) / \(totalDuration)")
